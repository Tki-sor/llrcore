package com.tkisor.llrcore.gtm.common.item;
...
public class TestingTerminalBehavior implements IInteractionItem {
    ...
    private void sendSuccessMessage(Player player) {
        player.sendSystemMessage(
                Component.translatable("llrcore.test_terminal.success").withStyle(ChatFormatting.GREEN));
    }
    ...
    private List<PatternError> check(IMultiController controller, BlockPattern pattern) {
        ...
        for (Direction direction : facings) {
            checkPatternAndCollectErrors(
                    pattern, errors, worldState, centerPos, direction, upwardsFacing, false);
            if (allowsFlip) {
                // 检查翻转状态
                worldState = new MultiblockState(worldState.getWorld(), worldState.getPos());
                checkPatternAndCollectErrors(
                        pattern, errors, worldState, centerPos, direction, upwardsFacing, true);
            }
        }
    ...
    private void checkPatternAndCollectErrors(
                                              BlockPattern pattern, List<PatternError> errors, MultiblockState worldState, BlockPos centerPos, Direction direction, Direction upwardsFacing, boolean flip) {
        ...
        if (error instanceof SinglePredicateError) {
            ...
            messages.add(
                    Component.translatable(
                            "llrcore.test_terminal.lack_error", Component.translatable(
                                    "llrcore.test_terminal.position", pos.getX(), pos.getY(), pos.getZ())));
            messages.add(
                    Component.literal(" - ").append(root).append(
                            Component.translatable(
                                    "llrcore.test_terminal.error_info", error.getErrorInfo())));
        } else {
            messages.add(
                    Component.translatable(
                            "llrcore.test_terminal.wrong_error", Component.translatable(
                                    "llrcore.test_terminal.position", pos.getX(), pos.getY(), pos.getZ())));
            List<List<ItemStack>> candidates = error.getCandidates();
